                ...
                <div style="display: none;padding: 5px" id="addOrUpdateWindow">
                    <form class="layui-form" style="width:90%;" id="dataFrm" lay-filter="dataFrm">
                        ...
                        <div class="layui-form-item">
                            ...
                            <div class="layui-input-block">
                                <input type="text" name="menu_title" lay-verify="required" autocomplete="off" placeholder="请输入菜单名称" class="layui-input">
                            </div>
                        </div>
                        <div class="layui-form-item">
                            <label class="layui-form-label">菜单地址</label>
                            <div class="layui-input-block">
                                <input type="text" name="menu_href"  autocomplete="off" placeholder="请输入菜单地址" class="layui-input">
                            </div>
                        </div>
                        <div class="layui-form-item">
                        ...
                        <div class="layui-form-item">
                            <div class="layui-inline">
                                ...
                                <div class="layui-input-block">
                                    <input type="radio" name="menu_spread" value="1" title="是">
                                    <input type="radio" name="menu_spread" value="0" title="否" checked>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        layui.extend({
            ...
            var tableIns = table.render({
                ...
                toolbar: '#toolbarDemo',
                cols: [[
                    {field: 'menu_id', width: 100, title: "菜单编号",align: 'center'},
                    {field: 'menu_title', width: 180, title: '菜单名称',align: 'center'},
                    {field: 'menu_href', minWidth: 200, title: '菜单地址',align: 'center'},
                    {field: 'menu_spread', width: 100, title: '是否展开',align: 'center',templet:function(d) {
                        return d.menu_spread == 1 ? "<font color='red'>是</font>" : "<font color='blue'>否</font>";
                    }},
                    {field: 'menu_icon', width: 100, title: '菜单图标',align: 'center',templet:function(d) {
            ...
            dtree.on("node(menuTree)",function(obj) {
                ...
                tableIns.reload({
                    where:{
                        "menu_id":obj.param.nodeId //其中nodeId属性就是选中节点的id值
                    },
                    page:{curr: 1}
                });
            });
            ...
            */
            function openUpdateWindow(data) {
                mainIndex = layer.open({
                    ...
                    success: function () {
                        ...
                        //图标回显
                        iconPickerFa.checkIcon('iconPicker', data.menu_icon);
                        //父级菜单回显
                        dtree.dataInit("menuSelectTree",data.menu_pid);//参数1：下拉菜单树的id属性值,参数2：父节点ID值
                        dtree.selectVal("menuSelectTree");//参数1：下拉菜单树的id属性值
                        //判断当前选中节点是否是一级菜单，如果是一级菜单，那么该菜单是没有父级菜单，此时应该显示请选择
                        if(data.menu_pid==0){
                           //刷新菜单树
                           //如果不刷新的话，点了其他子菜单的父菜单，再点其他子菜单的话会依旧保持上一个子菜单的父菜单
			...
			iconPickerFa.render({
				...
				click: function (data) {
					...
					//给图标隐藏域赋值(data.icon,不是data.menu_icon)
                    $("#menu_icon").val("fa "+data.icon);
				},
				// 渲染成功后的回调
				success: function (d) {
					console.log(d);
            ...
            */
            function deleteById(data) {
                $.get("/admin/menu/checkMenuHasChild",{"menu_id":data.menu_id},function(result){
                    //如果该菜单存在子菜单，则进行提示；如果没有子菜单，则提示是否删除
                    if(result.exist){
                        layer.msg(result.message);
                    }else{
                        //提示用户是否删除该菜单
                        layer.confirm("确定要删除[<font color='red'>"+data.menu_title+"</font>]吗", {icon: 3, title:'提示'}, function(index){
                        //发送ajax请求进行删除
                        $.post("/admin/menu/deleteById",{"menu_id":data.menu_id},function(result){
                        if(result.success){
                        //刷新数据表格
                        tableIns.reload();
                        //刷新左侧菜单树
