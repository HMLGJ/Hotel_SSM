        ...
        <div class="layui-container layui-hide-xs">
            <div class="product-list">
                <dl id="getIndexRoomType">
                    ...
                    <!-- roomTypeList是IndexController里的模型数据, -->
                    <c:forEach var="roomType" items="${roomTypeList}">
                        <dd data-id="${roomType.roomType_id}">
                        	<!-- 根据id查询房型，跳转到房型页面 -->
                            <a class="fly-case-active" href="/room/list/${roomType.roomType_id}"
                               data-type="toRoomTypeList">${roomType.roomType_name}</a>
                        </dd>
                    </c:forEach>
                </dl>
            </div>
        ...
        <div class="shop-temp" id="getIndexFloor">
            ...
            <c:forEach var="floor" items="${floorList}" varStatus="stauts">
                ...
                <c:if test="${stauts.index%2!=0}">
                    ...
                    <div class="layui-container">
                    	<!-- 解决酒店楼层小bug排序问题，假如我数据库3楼写在2楼前面，那么我数据库需要添加一个sort排序列，然后在DAO.xml根据sort这个列排序 -->
                        <p class="temp-title-cn"><span></span>酒店${floor.floor_name}<span></span></p>
                        <div class="layui-row layui-col-space20">
                            <c:forEach var="room" items="${roomList}">
                            	<!-- 如果相等则显示，不相等则不显示 -->
                                <c:if test="${room.room_floorid == floor.floor_id}">
                                    <div data-id="${room.room_id}" class="layui-col-xs6 layui-col-md3">
                                        <a class="template store-list-box fly-case-active"
                                           href="/room/${room.room_id}" data-type="toRoomInfo">
                                            <img src="/hotel/show/${room.room_photo}" class="store-list-cover">
                                            <h2 class="layui-elip">${room.room_typeName}</h2>
                                            <p class="price">
                                                <span title="金额"> ￥${room.room_price} </span>
                                                <span title="房间号" style="color:  #fff;
                                                background: #0e88cc;padding: 3px;text-align: center;border: 1px solid #4cffb3;
                                                font-size: 13px;"> NO.${room.room_roomnum} </span>
                                            </p>
                                        </a>
                                    </div>
                                </c:if>
