...
<div class="layui-container shopdata">
        ...
        <div class="layui-card-header">
				<span class="layui-breadcrumb layui-hide-xs" style="visibility: visible;">
                    ...
                    <a href="/room/hotelList.html"
                       id="floorNumber">酒店${room.room_floorName}</a><span lay-separator="">/</span>
						 <a><cite>房间详情</cite></a>
                </span>
        </div>
        <div class="layui-card-body layui-row">
            <div class="layui-col-md6">
                <div class="intro-img photos">
                    <img id="coverImg" src="/hotel/show/${room.room_photo}" alt="产品封面" layer-index="0"> </div>
            </div>
            <div class="layui-col-md6">
                <div class="intro-txt">
                    <h1 class="title" id="roomName">${room.room_typeName}</h1>
                    <input type="hidden" id="room_id" name="room_id" value="1">
                    <div class="store-attrs">
                        <div class="summary">
                            <p class="reference"><span>房间号</span> <span id="roomNumber">NO.${room.room_roomnum}</span></p>
                            <p class="reference"><span>床位数</span> <span id="bedType">${room.room_dednum}张</span></p>
                            <p class="reference"><span>宽　带</span> <span id="broadband">免费wifi</span></p>
                            <p class="reference"><span>标准价</span> ￥<span id="standardPrice">${room.room_price}</span></p>
                            <p class="activity"><span>会员价</span><strong class="price"><i>￥</i><span id="memberPrice">${room.room_price*0.88}</span></strong></p>
                            <p class="activity"><span>状&#12288;态</span>
                            <strong class="status">

                                 <span>${room.room_statusStr}</span>
                    ...
                    <p class="store-detail-active" id="shopEvent">
                        <%-- 隐藏域，保存当前登录用户的ID值 --%>
                        <input type="hidden" id="currentUserId" value="${sessionScope.currentUser.user_id}">
                        <%-- 隐藏域，保存房间价格 --%>
                        <input type="hidden" id="room_price" value="${room.room_price}">
                        <c:if test="${room.room_status==3}">
    ...
    <div class="layui-card shopdata-content">
        <div class="layui-card-body detail-body layui-text">
            <div class="layui-elem-quote"> ${room.room_roomrequirement}</div>
            <div id="roomContent">
                ${room.room_roomdesc}
            </div>
        </div>
...
<div style="display: none;padding: 5px" id="orderRoomWindow">
    <form class="layui-form" style="width:90%;" id="dataFrm" lay-filter="dataFrm">
        <%-- 添加3个隐藏域，给订单分别保存用户ID、房间ID、房型ID --%>
        <input type="hidden" name="orders_accountid" value="${sessionScope.currentUser.user_id}">
        <input type="hidden" name="orders_roomid" value="${room.room_id}">
        <input type="hidden" name="orders_roomtypeid" value="${room.room_roomtypeid}">
        ...
        <div class="layui-form-item">
            <div class="layui-inline">
                ...
                <div class="layui-input-inline">
                    <input type="text" name="orders_arrivedate" id="arrivedate" lay-verify="required"  readonly placeholder="请选择入住时间"
                           class="layui-input">
                </div>
            </div>
            <div class="layui-inline">
                ...
                <div class="layui-input-inline">
                    <input type="text" name="orders_leavedate" id="leavedate" lay-verify="required" readonly  placeholder="请选择离店时间"
                           class="layui-input">
                </div>
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <div class="layui-inline">
                ...
                <div class="layui-input-inline">
                    <input type="text" name="orders_reserveprice" id="reservePrice" lay-verify="required"  readonly placeholder="房费价格自动计算"
                           class="layui-input">
                </div>
            </div>
        </div>
        <div class="layui-form-item">
            <div class="layui-inline">
                ...
                <div class="layui-input-inline">
                    <input type="text" name="orders_reservationname" lay-verify="required"  placeholder="请输入预订人姓名"
                           class="layui-input">
                </div>
            </div>
            <div class="layui-inline">
                ...
                <div class="layui-input-inline">
                    <input type="text" name="orders_phone" lay-verify="required"  placeholder="请输入预订人电话"
                           class="layui-input">
                </div>
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" name="orders_idcard" lay-verify="required" autocomplete="off" placeholder="请输入身份证号码"
                       class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <textarea class="layui-textarea" name="orders_remark" id="content"></textarea>
            </div>
        </div>
...
<div class="fly-footer">
    <p><a href="#">酒店管理系统</a> 2021 ©
        <a href="" target="_blank">gitee地址：123456</a>
    </p>
</div>
...
<script>
    ...
    layui.use(["form","element","carousel","layer","laydate"], function () {
        ...
        laydate.render({
            elem:"#book_date",
            min:"new Date()",//限定开始日期,-3就是可预订当前日期3天前，new Date()则是当前时间
            range:"至",//选择范围，内容可以自定义,可以写成~,也可以写成-
            //运用layui官方文档的时间组件的回调的选择完毕的回调,还有最大/最小日期
            done:function (value,date,endDate) {
                //var startDate = value.substr(0,value.indexOf("至")).trim();//因为 至 之前有一个空格
                //var endDate = value.substr(value.indexOf("至")+1,value.length).trim();//因为 至 之后有一个空格
                //截时间,.trim()是去空格的方法。
        ...
        $("#bookRoomBtn").click(function () {
            ...
            if(userId=="" || userId.length==0){
                //layer.alert("您还没有登录，请先登录！");//这个优先级不高，直接跳到登录页面了，反应不过来看提示
                alert("您还没有登录，请先登录！")
