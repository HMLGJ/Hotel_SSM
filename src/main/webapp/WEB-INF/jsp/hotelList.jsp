            ...
            <div class="layui-card-body">
                <div class="store-cat-item"> <span><i class="layui-icon layui-icon-shop-fenlei"></i>类别：</span>
                    <ul id="getAllRoomType">
                    	<!-- typeId是在RoomController控制类里面定义的模型 -->
                        ...
                        <c:forEach var="roomType" items="${roomTypeList}">
                            ...
                            <li data-id="${roomType.roomType_id}"
                                <c:if test="${typeId ==  roomType.roomType_id}">class="active"</c:if>>
                                <a class="fly-case-active"
                                   href="/room/list/${roomType.roomType_id}"
                                   data-type="toRoomTypeListByLists">${roomType.roomType_name}</a> </li>
                        </c:forEach>
                    </ul>
                </div>
        ...
        <div class="layui-container">
            <div class="layui-row layui-col-space20 shoplist" id="roomList">
                <c:forEach var="room" items="${roomList}">
                    <div data-id="${room.room_id}" class="layui-col-xs12 layui-col-sm6 layui-col-md4 layui-col-lg3">
                    <a class="template store-list-box fly-case-active" href="/room/${room.room_id}" data-type="toRoomInfo">
                        <img src="/hotel/show/${room.room_photo}" class="store-list-cover">
                        <h2 class="layui-elip">${room.room_typeName}</h2>
                        <div> <label class="layui-badge-rim store-list-pay"> ￥${room.room_price} </label>
                            <div class="store-list-colorbar">
                                <span class="store-color-bar" style="position:relative;bottom:3px;border-color: #009688;color: #009688;border-width: 1px;border-style: solid;background-color: #fff;    text-align: center;">NO.${room.room_roomnum}</span>
                                <span class="store-color-bar" style="border-color: #5fb878;color: #5fb878;border-width: 1px;border-style: solid;background-color: #fff;    text-align: center;">${room.room_typeName}</span>
                                <span class="store-color-bar" style="border-color: #01aaed;color: #01aaed;border-width: 1px;border-style: solid;background-color: #fff;    text-align: center;">${room.room_floorName}</span>
                            </div>
                        </div>
                    </a>
                </div>
